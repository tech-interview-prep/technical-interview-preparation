...
class SumRootToLeafNo {
    ...
    public static int helper(TreeNode root, int x) {
        if (root.right == null && root.left == null) return 10 * x + root.data;
        
        int val = 0;
        if (root.left != null) val += helper(root.left, 10 * x + root.data);
        if (root.right != null) val += helper(root.right, 10 * x + root.data);
        return val;
    }
}
